...
class MY_Controller extends CI_Controller
{
    ...
    public function cargarComponentesform($datosForm)
    {
        ...
        $html = '<form action="'.base_url().index_page().'/admin/Cargatablas/exportarParametros/'.$datosForm[0]->tabla_nombre.'" method="'.$datosForm[0]->tabla_method.'">';
            ...
            foreach ($datosForm as $key) {
                switch ($key->tipocampo_nombre) 
                {
                    case 'input':
                        ...
                        elseif (strpos($key->campo_value, '{{')) {
                            $retValValue = $this->{$key->campo_value};
                        }elseif (!strpos($key->campo_value, '[[')) {
